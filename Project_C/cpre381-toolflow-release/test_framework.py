...
def main():
	...
	if not modelsim.is_installed():
		print('\nModelsim does not seem to be installed in the expected location: C:/modelsim_dlx64_10.7b/win64pe')
		print('program is exiting\n')
		exit()
...
def parse_args():
	...
	parser.add_argument('--sim-timeout',type=check_sim_timeout, default=2000000, help='change the ammount of time before simulation is forcefully stopped')
...
expected_tb_checksum = b'r\xe2\xdap\x9e\x0f\xe6s\xf9\xf4&\xe1\xd3.\xe5\xb8'
...
def warn_tb_checksum():
	...
	Assumes file exists.
	'''
	expected = b'r\xe2\xdap\x9e\x0f\xe6s\xf9\xf4&\xe1\xd3.\xe5\xb8'

	with open('ModelSimWork/src/tb_SimplifiedMIPSProcessor.vhd','rb') as f:
		observed = hashlib.md5(f.read()).digest()

	if observed != expected:
		print('** Warning: It looks like ModelSimWork/src/tb_SimplifiedMIPSProcessor.vhd has been modified. It will be graded using the version from the release **')
